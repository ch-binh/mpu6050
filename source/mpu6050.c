 ...
 * @details Implements initialization, configuration, and data access functions
 *          for the MPU6050 accelerometer and gyroscope sensor.
 *
 * @date    2025/07/08
 * @author  Binh Pham
 * @version 1.0.0
...
void mpu6050_init_reg(void)
{
  ...
  mpu6050_cfg_int_pin(DEF_INT_LVL, DEF_INT_OP, DEF_EN_LATCH, DEF_INT_CLR_MODE, DEF_FSYNC_LV,
                      DEF_DIS_FSYNC_INT, DEF_DIS_I2C_BYPASS);
...
{
  mpu6050_write_reg(REG_SMPRT_DIV, val);
...
{
  mpu6050_write_reg(REG_CFG, (fsync << 3) | dlpf);
...
void mpu6050_cfg_gyro(uint8_t en_gyr_test, uint8_t fs_sel)
{
  ...
  mpu6050_write_reg(REG_GYRO_CFG, en_gyr_test_val | (fs_sel << 3));
}
...
void mpu6050_cfg_accel(uint8_t en_acc_test, uint8_t fs_sel)
{
  ...
  mpu6050_write_reg(REG_ACCEL_CFG, en_acc_test_val | (fs_sel << 3));
}

/* REG 0x1F */
void mpu6050_cfg_set_mot_thr(uint8_t val)
{
  mpu6050_write_reg(REG_MOT_THR, val);
  _cfg.mot.thr = val;
}
...
{
  mpu6050_write_reg(REG_FIFO_EN, val);
}
...
{
  mpu6050_write_reg(REG_INT_PIN_CFG, (int_lvl << 7) | (int_open << 6) | (is_latch << 5) |
                                       (int_clr_mode << 4) | (fsync_int_lv << 3) | (fsync_int_en << 2) |
                                       (i2c_bypass_en << 1));
}
...
{
  mpu6050_write_reg(REG_INT_EN, (mot << 6) | (ffull << 4) | (i2c_mst_int << 3) | (data_rdy_int));
}

/* REG 0x6B*/
void mpu6050_cfg_pwr_mng(uint8_t mode)
{
  mpu6050_write_reg(REG_PWR_MGMT_1, mode);
}
...
int mpu6050_get_fs(mpu_cfg_t *mpu_cfg)
{
  ...
  mpu6050_read_reg(REG_GYRO_CFG, val, size);
...
int mpu6050_get_int_sts(void)
{
  ...
  mpu6050_read_reg(REG_INT_STS, val, size);
  return val[1];
}
...
uint8_t mpu6050_get_accel_raw(mpu_rawdata_t *r_data)
{
  ...
  mpu6050_read_reg(REG_ACCEL_XOUT_MSB, val, size); // Read all at once
...
uint8_t mpu6050_get_temp_raw(mpu_rawdata_t *r_data)
{
  uint8_t size = 2;
  uint8_t val[size];
  mpu6050_read_reg(REG_TEMP_MSB, val, size);
...
{
  uint8_t val[6];                              // Buffer for all 6 accelerometer bytes
  mpu6050_read_reg(REG_GYRO_XOUT_MSB, val, 6); // Read all at once
...
int mpu6050_get_chip_id(void)
{
  ...
  mpu6050_read_reg(REG_WHO_AM_I, values, 1);
  return values[0];
}
